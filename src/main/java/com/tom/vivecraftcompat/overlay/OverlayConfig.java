...
public class OverlayConfig {
	...
	public static void loadOverlays() {
		...
		ovs.keySet().forEach(key -> {
			...
			// Load rotation (using float4 for quaternion)
	...
	public static void saveOverlays() {
		...
		OverlayManager.forEachLayer(l -> {
			if (l.getScreen() instanceof HudOverlayScreen h) {
				...
				rotationMatrix.getUnnormalizedRotation(quaternion);  // Convert matrix to quaternion

				// Store the quaternion components (x, y, z, w)
